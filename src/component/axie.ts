...
import { SkeletonJson, AtlasAttachmentLoader, Spine } from "@pixi-spine/runtime-3.8"
...
export class Figure extends Spine {
  ...
  static async loadAndSpawn(loader: PIXI.Loader, mixer: Mixer) {
    await this.loadResources(loader, mixer)
    return new Figure(mixer)
  }

  static async fromAxieId(loader: PIXI.Loader, id: string) {
    try {
  ...
  static async fromMixer(loader: PIXI.Loader, mixer: Mixer) {
    const newFigure = await this.loadAndSpawn(loader, mixer)
    newFigure.stateData.setMix("draft/run-origin", "action/idle/normal", 0.1)
    newFigure.stateData.setMix("action/idle/normal", "draft/run-origin", 0.2)
    return newFigure
  ...
  static async loadResources(loader: PIXI.Loader, mixer: Mixer) {
    loader.reset()
